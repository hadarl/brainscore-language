...
def load_t17speech2024_reference():
    # assembly = load_t17speech2024_reference()

    # data_dir = "../../../DATA/willett2023/competitionData/train/"
    #data_dir = "/mnt/t17_brainscore/neuralData/"

    allSessionData_file = open('/home/hadarla/Projects/T17_brand/code/brand-mat-analysis/session_analysis_notebooks/allSessionData_trialStart1.pkl', 'rb')
    allSessionData = pickle.load(allSessionData_file)

    len(allSessionData)

    # inputFeatures_allSessions = []
    # transcriptions_allSessions = []
    # for s in range(len(allSessionData)):
    #
    #     inputFeatures = allSessionData[s]['inputFeatures']
    #     transcriptions = allSessionData[s]['transcriptions']

        # minsz = np.shape(inputFeatures[0])[0]
        # for x in inputFeatures:
        #     if np.shape(x)[0] < minsz:
        #         minsz = np.shape(x)[0]
        # # print(minsz)

        # inputFeatures_backup = inputFeatures

        # inputFeatures_sameSize = []
        # for x in inputFeatures:
        #     inputFeatures_sameSize.append(x[0:minsz, :])

        # # np.shape(inputFeatures_sameSize)

        # inputFeatures = inputFeatures_sameSize

        # np.shape(inputFeatures)
        # [trials, time points, features] ?

        # inputFeatures_allSessions.append(np.array(inputFeatures))
        # transcriptions_allSessions.append(transcriptions)

    #transcriptions_allSessions_concat = np.concatenate(transcriptions_allSessions)

    # num_timesteps = 400

    # inputFeatures_allSessions_minSize = inputFeatures_allSessions[0][:, 0:num_timesteps, :]
    # for s in range(1, len(inputFeatures_allSessions)):
    #     inputFeatures_allSessions_minSize = np.concatenate(
    #         (inputFeatures_allSessions_minSize, inputFeatures_allSessions[s][:, 0:num_timesteps, :]), axis=0)
...
#    for data_file_name in tqdm(mat_files, desc="Session"):
    for s in tqdm(range(len(allSessionData)), desc="Session"):
        ...
        data = inputFeatures  # Currently the data contains both spike power (256-512 and TX (0-256)
        # spike_power = np.transpose(data['spikePow'])


        #sentences = [sentence.item() for sentence in data['sentenceText']]
        sentences = [sentence.item() for sentence in transcriptions]

        stim = load_t17speech2024_stimuli(sentences)

        # assembly
        #data['examples'] = data['spikePow']
        spike_power_data = load_t17speech2024_spikePow(data, sentences, stim)
        neural_data_per_word = spike_power_data['word_neural_data']

        # meta = data['__header__']

        electrode_numbers = list(range(np.shape(data[0])[1]))
        ...
        for i in range(np.shape(sentences)[0]):
            ...
            block_index_for_sentence.extend([i] * nWords)
        ...
        assembly = (NeuroidAssembly
                    (neural_data_per_word_T,
                     ...
                     coords={
                         ...
                         'session_num': ('presentation', [s] * len(neural_data_per_word)),
                         'session_index': ('presentation', [s] * len(neural_data_per_word)),
                         'subject': ('presentation', [s] * len(neural_data_per_word)),
...
def load_t17speech2024_stimuli(sentences):  # Stimuli
...
def load_t17speech2024_spikePow(data, sentences, stim):
    ...
    for i in range(np.shape(sentences)[0]):
        ...
        nNeural_samples_per_sentence = np.shape(data[i])[0]
        ...
        for j in range(nWords_in_sentence):
            ...
            word_neural_data.append(data[i][time_bin_start[-1]:time_bin_end[-1], :])
